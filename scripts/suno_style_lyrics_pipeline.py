...
def _word_level_reconstruct(asr_text: str, reference_lines: list) -> str:
    """
...
def align_lyrics_to_reference(asr_lines: list, reference_lines: list) -> list:
    ...
    # كل مرشح مربوط برقم بيته: bait_id = i // 2 لأن كل بيت = عنصرين متتاليين
    candidates = [
        (normalize_arabic(reference_lines[i]), reference_lines[i], i // 2)
    ...
    for i in range(0, len(reference_lines) - 1, 2):
        ...
        candidates.append((normalize_arabic(merged_line), merged_line, i // 2))

    corrected = []
    for line in asr_lines:
        norm = normalize_arabic(line["text"])
        scored = []
        for cand_norm, cand_orig, cand_bait_id in candidates:
            if not cand_norm:
                continue
            ratio = difflib.SequenceMatcher(None, norm, cand_norm).ratio()
            scored.append((ratio, cand_orig, cand_bait_id))
        scored.sort(key=lambda x: x[0], reverse=True)

        new_line = dict(line)
        new_line["bait_id"] = None
        if scored:
            best_ratio, best_text, best_bait_id = scored[0]
            ...
            if best_ratio > 0.55 and (best_ratio - second_ratio) > 0.05:
                ...
                # بدل ترك النص الخام غير المشكَّل كما هو. bait_id يبقى None
                # لأننا مو متأكدين لأي بيت ينتمي هالسطر بثقة.
                new_line["text"] = _word_level_reconstruct(line["text"], reference_lines)
